...
namespace CCC.Contracts.Registry.Services
{
    public class RegistryService
    {
        ...
        private string abi =
            @"[{'constant':false,'inputs':[{'name':'registeredAddress','type':'address'}],'name':'unregister','outputs':[],'payable':false,'type':'function'},{'constant':false,'inputs':[{'name':'registeredAddress','type':'address'}],'name':'register','outputs':[],'payable':false,'type':'function'},{'constant':true,'inputs':[{'name':'','type':'address'}],'name':'records','outputs':[{'name':'registeredAddress','type':'address'},{'name':'owner','type':'address'},{'name':'time','type':'uint256'},{'name':'Id','type':'uint256'}],'payable':false,'type':'function'},{'constant':true,'inputs':[{'name':'','type':'uint256'}],'name':'workRegistered','outputs':[{'name':'','type':'address'}],'payable':false,'type':'function'},{'constant':true,'inputs':[],'name':'numRecords','outputs':[{'name':'','type':'uint256'}],'payable':false,'type':'function'},{'constant':true,'inputs':[],'name':'maxId','outputs':[{'name':'','type':'uint256'}],'payable':false,'type':'function'},{'anonymous':false,'inputs':[{'indexed':true,'name':'registeredAddress','type':'address'},{'indexed':true,'name':'id','type':'uint256'},{'indexed':true,'name':'owner','type':'address'},{'indexed':false,'name':'time','type':'uint256'}],'name':'Registered','type':'event'},{'anonymous':false,'inputs':[{'indexed':true,'name':'registeredAddress','type':'address'},{'indexed':true,'name':'id','type':'uint256'}],'name':'Unregistered','type':'event'}]";

        private Contract contract;

        public RegistryService(Web3 web3, string address)
        {
            this.web3 = web3;
        ...
        public Function GetAddressRegisteredFunction()
        {
            return contract.GetFunction("workRegistered");
        }

        public Task<string> GetAddressRegisteredAsyncCall(long id)
        {
            var function = GetAddressRegisteredFunction();
